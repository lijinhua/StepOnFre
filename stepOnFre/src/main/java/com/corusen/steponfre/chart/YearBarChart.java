...
public class YearBarChart extends AbstractDemoChart {
	...
	public GraphicalView graphicalView(DemoObjectFragment demoObjectFragment, List<double[]> x, List<double[]> values,
		...
		String[] titles = new String[] { demoObjectFragment
				.getString(R.string.yearly_total) + ": " }; // WL change
...
//			large = true;
		...
		double maxStep = 0.0;
		double[] sumSteps = (double[]) x.get(0);
		double[] aveSteps = (double[]) values.get(0);

		for (int i = 0; i < aveSteps.length; i++) {
			if (aveSteps[i] > maxStep) {
				maxStep = aveSteps[i];
			}
		}
		
		totalSteps = (int) sumSteps[0];
		aveTotalAveSteps = (int) sumSteps[1];

		titles[0] = titles[0] + ((Integer) totalSteps).toString() + ", "
				+ demoObjectFragment.getString(R.string.ave) + ": "
				+ ((Integer) aveTotalAveSteps).toString();
...
//		int height = size.y;
		...
		renderer.addXTextLabel(1, demoObjectFragment.getString(R.string.jan));
		renderer.addXTextLabel(3, demoObjectFragment.getString(R.string.mar));
		renderer.addXTextLabel(5, demoObjectFragment.getString(R.string.may));
		renderer.addXTextLabel(7, demoObjectFragment.getString(R.string.jul));
		renderer.addXTextLabel(9, demoObjectFragment.getString(R.string.sep));
		renderer.addXTextLabel(11, demoObjectFragment.getString(R.string.nov));
		...
		renderer.setBarSpacing(0.3f);	//WL 1/23/12
		...
		return ChartFactory.getBarChartView(demoObjectFragment.getActivity().getBaseContext(),
				buildBarDataset(titles, values), renderer, Type.STACKED); // WL
																			// change
